# Code stolen from https://github.com/danpaquin/gdax-python
#
...
class PublicClient(object):
    ...
    def get_product_trades(self, product_id, before='', after='', limit=100,
                           result=[]):
        ...
        Args:
             ...
             limit (Optional[int]): the desired number of trades (can be more
                                    than 100, automatically paginated)
             results (Optional[list]): list of results that is used for the
                                        pagination
        Returns:
        ...
        **Caution**: Historical rates should not be polled frequently.
        ...
        if granularity is not None:
            ...
            if granularity not in acceptedGrans:
                newGranularity = min(acceptedGrans,
                                     key=lambda x:abs(x-granularity))
                print(granularity,' is not a valid granularity level, using',
                      newGranularity,' instead.')
